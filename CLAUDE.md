...
**Version:** 1.4.0 (see README.md for published version)
...
### Setup
```bash
# Install Composer dependencies (required for linting)
composer install
```
...
# - main: Production deployments (GitHub Actions auto-deploy)
# - feature/*: Feature development branches
# - Commits always pushed to remote (never left local-only)
...
### Caching Strategy
- **Company Feed**: 7 days (weekly cron refresh) - transient cache only, no object cache
...
- **Lock Mechanism**: 60-second lock prevents simultaneous fetches, with stale cache fallback
...
- Use conventional commit format: `feat:`, `fix:`, `docs:`, `refactor:`, `perf:`, `security:`
- Always push commits to remote (never leave local-only)
...
### Issue: RSS feeds timing out or locking up site
**Solution**: Transient-only caching with 60-second locks and stale cache fallback prevents simultaneous fetches
