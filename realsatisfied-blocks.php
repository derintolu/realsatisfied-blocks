...
class RealSatisfied_Blocks {
	...
	public function init() {
		...
		if ( class_exists( 'RealSatisfied_RSS_Cache_Manager' ) ) {
			RealSatisfied_RSS_Cache_Manager::get_instance();
		}

		// Initialize cache admin
		if ( class_exists( 'RealSatisfied_Cache_Admin' ) ) {
		...
		if ( ! $should_load_agent_script ) {
			...
			if ( $post ) {
				...
				if ( $post->post_type === 'person' ||
					get_post_meta( $post->ID, 'realsatisfied-agent-vanity', true ) ||
					( function_exists( 'get_field' ) && get_field( 'realsatisfied-agent-vanity', $post->ID ) ) ) {
					$should_load_agent_script = true;
				}
			}
		}
	 ...
	 */
	public function add_loading_lazy_to_realsatisfied_images( $value, $image ) {
		// Only apply to images with RealSatisfied classes
		if ( is_string( $image ) && (
			strpos( $image, 'realsatisfied-' ) !== false ||
